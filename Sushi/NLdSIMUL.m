classdef NLdSIMUL < handle
    ...
    methods
        ...
        function kl_value = kl(obj, offersets, sales)
            % function to compute the KL
            prob_matrix = obj.compute_proba(offersets);
            ...
            for row = 1:total_row
                ...
                value = value + sum(sale_row(sale_row~=0).*log(sale_row(sale_row~=0)./prob_row(sale_row~=0)) );
        ...
        function map_value = abss(obj, offersets, sales)
            ...
            total_row = size(offersets,1);
            for row = 1:total_row
                offersets_row = offersets(row,:);
                sale_row= sales(row,:);
        		prob_row= prob_matrix(row,:);

                %value = value + sum(abs((sale_row(sale_row~=0)-prob_row(sale_row~=0)))./sale_row(sale_row~=0))/nnz(sale_row);
                value = value + sum(abs(sale_row(sale_row~=0)-prob_row(sale_row~=0))./sale_row(sale_row~=0))/nnz(offersets_row);
            end
        ...
        function [chi_soft,chi_hard,mape_soft,mape_hard] = Ni(obj, offersets, sales)
            ...
            chi_soft = sum(((Ni-Ni_soft).^2)./(Ni_soft));
            chi_hard = sum(((Ni(Ni_hard~=0)-Ni_hard(Ni_hard~=0)).^2)./(Ni_hard(Ni_hard~=0)));
            
            mape_soft = sum(abs(Ni-Ni_soft)./(Ni));
            mape_hard = sum(abs(Ni-Ni_hard)./(Ni));
        ...
        function [op,mu, lambda] = train(obj, offersets, sales, mu_lb, mu_ub, lambda_lb, method, number, init_mnl)
            if(strcmp(method, 'gdmu'))
                [op,mu, lambda] = obj.fit_gdmu(offersets, sales, mu_lb, mu_ub, lambda_lb,number, init_mnl);
            elseif(strcmp(method, 'gdmu_mnl'))
                [op,mu, lambda] = obj.fit_gdmu_mnl(offersets, sales, mu_lb, mu_ub, lambda_lb);
            elseif(strcmp(method, 'fullback_mm'))
                [op,mu, lambda] = obj.fit_fullback_mm(offersets, sales, mu_lb, mu_ub, lambda_lb,number, init_mnl);
            elseif(strcmp(method, 'fullback_mnl'))
                [op,mu, lambda] = obj.fit_fullback_mnl(offersets, sales, mu_lb, mu_ub, lambda_lb);
            elseif(strcmp(method, 'knitro'))
                [op,mu, lambda] = obj.fit_knitro(offersets, sales, mu_lb, mu_ub, lambda_lb, number, init_mnl);
	    elseif(strcmp(method, 'knitro_mnl'))
                 [op,mu, lambda] = obj.fit_knitro_mnl(offersets, sales, mu_lb, mu_ub, lambda_lb);
            end            
        end  
...
%                 x = fminsearch(f, x0);
        ...
        function [fval,mu, lambda] = fit_knitro(obj, offersets, sales, mu_lb, mu_ub, lambda_lb, number, init_mnl)
...
%             options = optimoptions('fmincon', 'Display', 'off', 'MaxFunEvals',50000);
            ...
            if init_mnl
                load(['ground_truth_mnl/',num2str(number),'.mat']);
                obj.mean_utils = ground_truth_m_2.mean_utils;
            else
                obj.mean_utils = zeros(1, obj.num_prods);
            end
            x0 = [obj.mean_utils, ones(1, num_non_leaf_nodes)];
            %options = knitro_options('outlev', 4, 'maxit', 400, 'maxtime_real', 7e3, 'algorithm', 5);
	        options = knitro_options('outlev', 4, 'maxit', 50, 'algorithm', 5, 'ma_terminate', 0);
...
%             end
            ...
            mu = obj.mean_utils;
            lambda = obj.nest_dissim_params;
            %[x, fval, exitflag, output] = fmincon(f, zeros(1,obj.num_prods+obj.num_nests), [], [], [], [], lb, ub, []);            
        end %knitro       
        

        function [fval,mu, lambda] = fit_knitro_mnl(obj, offersets, sales, mu_lb, mu_ub, lambda_lb)
            f = @(x) obj.opt_func(x, offersets, sales);
            
            num_non_leaf_nodes = sum(obj.num_nodes_per_level) - obj.num_prods;
            %lb = [mu_lb*ones(1,obj.num_prods), ones(1,num_non_leaf_nodes-1), 1];
            lb = [mu_lb*ones(1,obj.num_prods), ones(1,num_non_leaf_nodes)];
            ub = [(Inf)*ones(1,obj.num_prods), ones(1,num_non_leaf_nodes)];
            %lb(1) = 0;
            %ub(1) = 0;
            Aleq = [];
            ...
            %options = knitro_options('outlev', 4, 'maxit', 400, 'maxtime_real', 7e3, 'algorithm', 5);
	        options = knitro_options('outlev', 4, 'maxit', 400, 'algorithm', 5, 'ma_terminate', 0);
        ...
        function [fval,mu, lambda] = fit_gdmu(obj, offersets, sales, mu_lb, mu_ub, lambda_lb, number, init_mnl)
            ...
            if init_mnl
                load(['ground_truth_mnl/',num2str(number),'.mat']);
                obj.mean_utils = ground_truth_m_3.mean_utils;
            else
                % this is equivalent to setting all mus to zero
                obj.mean_utils = ones(1, obj.num_prods);
            end
            %x0 = [ground_truth_m_3.mean_utils, ones(1, num_non_leaf_nodes)];
            ...
            % compute sales at each node in the tree and initialize the
                % nest dissimilarity parameters of the model
                node_sales = {sales};
                for level=1:obj.num_levels
...
%                         end            
                    ...
                    options = knitro_options('maxit', 800, 'maxtime_real', 5e2);
...
%             end
        ...
        function [fval,mu, lambda] = fit_gdmu_mnl(obj, offersets, sales, mu_lb, mu_ub, lambda_lb)
            % uses gd algo to estimate the parameters of the model
            num_non_leaf_nodes = sum(obj.num_nodes_per_level) - obj.num_prods;
...
%                         end            
                    ...
                    f_projection = @(y) norm(y-xnew')^2;
                    y0 = [zeros(1,obj.num_prods), zeros(1,num_non_leaf_nodes-1), 1]';
                    [xpro, fval, exitflag, output, lambda, grad, hessian] = knitro_nlp(f_projection, y0, [], [], [], [], lb, ub, [],[], options);
            ...
            mu = obj.mean_utils;
            lambda = obj.nest_dissim_params;
            fprintf('GD mu NLL MNL Algo. termininated in %d iters...obj val %f\n', niter, fval);
...
%             end
        ...
        end %gd back tracking 1d NLL mu lambda MNL
        
        function [fval,mu, lambda] = fit_fullback_mm(obj, offersets, sales, mu_lb, mu_ub, lambda_lb, number, init_mnl)
            ...
            if init_mnl
                load(['ground_truth_mnl/',num2str(number),'.mat']);
                obj.mean_utils = ground_truth_m_1.mean_utils;
            else
                % this is equivalent to setting all mus to zero
                obj.mean_utils = ones(1, obj.num_prods);
            end
            ...
            mu = obj.mean_utils;
            lambda = obj.nest_dissim_params;
            fprintf('Fullback Lin 1d Algo. termininated in %d iters...obj val %f\n', niter, fval);
        end % mm on mu, pgd on delta backtracking 1d, F1-linearF2         
             
        function [fval,mu, lambda] = fit_fullback_mnl(obj, offersets, sales, mu_lb, mu_ub, lambda_lb)
            ...
            % compute sales at each node in the tree and initialize the
                ...
                for level=1:obj.num_levels
                    node_sales{level+1} = node_sales{level}*obj.adj_mats{level};
                    obj.nest_dissim_params{level} = 1*ones(obj.num_nodes_per_level(level+1), 1);
                end
                obj.nest_dissim_params{obj.num_levels} = 1;
            ...
            mu = obj.mean_utils;
            lambda = obj.nest_dissim_params;
            fprintf('Fullback Lin MNL 1d Algo. termininated in %d iters...obj val %f\n', niter, fval);
        end % mm on mu, pgd on delta backtracking 1d, F1-linearF2 
    end 
end
